...
function loadFromFiles() {
    ...
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        // Development mode - use file server
        return loadFromDevelopmentFiles();
    } else {
        // Production mode (GitHub Pages) - use localStorage data manager
        return loadFromGitHubPages();
    }
}

function loadFromDevelopmentFiles() {
    let loadPromises = [];
...
function loadFromGitHubPages() {
    // Use GitHub Pages data manager
    return new Promise((resolve) => {
        if (window.githubDataManager) {
            // Load data from GitHub Pages data manager
            users = window.githubDataManager.getUsers();
            jobs = window.githubDataManager.getJobs();
            applications = window.githubDataManager.getApplications();
            savedJobs = window.githubDataManager.getSavedJobs();
            ...
            console.log('GitHub Pages: Data loaded from localStorage manager');
            console.log('Users:', users.length, 'Jobs:', jobs.length);
        }
        resolve();
    });
}
...
function saveToDevelopmentFiles() {
    // Save jobs to file
...
function saveToGitHubPages() {
    // Use GitHub Pages data manager
    if (window.githubDataManager) {
        window.githubDataManager.data.users = users;
        window.githubDataManager.data.jobs = jobs;
        window.githubDataManager.data.applications = applications;
        window.githubDataManager.data.savedJobs = savedJobs;
        
        window.githubDataManager.saveUsers();
        window.githubDataManager.saveJobs();
        window.githubDataManager.saveApplications();
        window.githubDataManager.saveSavedJobs();
        
        console.log('GitHub Pages: Data saved to localStorage manager');
    }
}
